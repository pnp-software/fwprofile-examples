...
{
	printf("  Action Action5\n");
}

/** Action Action6 */
void Action6(FwSmDesc_t smDesc)
{
	printf("  Action Action6\n");
}
...
int main(int argc, char* argv[])
{
	...
	FwSmDesc_t smDesc = Example2Create(NULL);

	/** Check that the SM is properly configured */
	if (FwSmCheckRec(smDesc) != smSuccess) {
		printf("The state machine Example2 is NOT properly configured ... FAILURE\n");
		return EXIT_FAILURE;
	}

	printf("The state machine Example2 is properly configured ... SUCCESS\n");
